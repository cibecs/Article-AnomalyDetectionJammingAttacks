...
As recall was the metric that we wanted to prioritize as highlights the number of detections, we chose the value of \textbf{contamination = 0.09}, as it provided the best metric values with our dataset.

Our testing showed that, as expected, tuning of the contamination parameter had a minimal impact on the training and classification time. 

\subsection{Model Testing Phase}

\subsubsection{Standard model testing}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{CCC}
    \toprule
    \textbf{Metric} & \textbf{Tuned Model} & \textbf{Default Model} \\
    \midrule
    \textbf{TP} & 1997 & 2000 \\
    \textbf{FP} & 1565 & 1993 \\
...
\begin{table}[H]
    ...
    \textbf{Metric} & \textbf{Tuned Model} & \textbf{Default Model} \\
    \midrule
    \textbf{Accuracy} & 0.93 & 0.91 \\
    \textbf{Precision} & 0.56 & 0.50 \\
    \textbf{Recall} & 1.00 & 1.00 \\
    \textbf{F1 Score} & 0.72 & 0.67 \\
    \bottomrule
    \end{tabularx}
\end{table}

The tuning phase proved effective in improving the model's performance compared to the default hyperparameters. Still, despite reaching a Recall score of 1.00 in both scenarios, the model is still too prone to false positives, as indicated by the value of the Precision metric. 
...
\subsubsection{Majority rule model testing}
...
\begin{figure}[H]
	\includegraphics[width=10.5 cm]{img/ModulesStructure.jpg}
	...
	\begin{algorithmic}[1]
		...
		\For{each index in range(len(classification))}
			...
			\If{current classification is OUTLIER and the count of OUTLIERS in the window 
				 is less than or equal to half the window size}
