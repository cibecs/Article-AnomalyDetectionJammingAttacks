...
When choosing the algorithm for the classification module, we first had to define the requirements that the algorithm had to meet. The algorithm had to be able to classify anomalous samples with a high degree of accuracy, while at the same time 
...
Unlike traditional anomaly detection algorithms, which require the definition of a normal class, Isolation Forest is able to detect anomalies without explicitly defining 
This is achieved by leveraging the properties of anomalyes themselves, i.e being few and separated from the majority of the data points. 
