...
In figure \ref{fig:InputSignal} we can see a representation of the input signal in the case of a constant jamming attack.
...
As we can see from graphs \ref{fig:n_estimators_training_time} and \ref{fig:n_estimators_classification_time}, the time required both for training and classification increases linearly with the number of estimators.
This means that choosing an appropriate number of estimators is crucial, as it can greatly affect the model performance. 

\begin{figure}[H]
    \centering
    \subfigure[n\_estimators training time.]{
        \includegraphics[width=0.45\textwidth]{img/nEstimatorsTrainingTime.png}
        \label{fig:n_estimators_training_time}
    }
    \hfill
    \subfigure[n\_estimators classification time.]{
        \includegraphics[width=0.45\textwidth]{img/nEstimatorsClassificationTime.png}
...
We decided to choose the value of 15 for the \textit{n\_estimators} parameter, as it provided a good balance between model performance and computational resources required.
